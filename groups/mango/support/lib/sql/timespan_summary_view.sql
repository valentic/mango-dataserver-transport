CREATE OR REPLACE VIEW timespan_summary AS
...
WITH dailysummary as (
    ...
    FROM
        image_islands
    WHERE
        timestamp::date >= '2024-10-1' and timestamp::date < '2025-10-1'::date
    GROUP BY
    ...
    WHERE
        ...
        AND
        image.timestamp BETWEEN '2024-10-1'::timestamptz and '2025-10-1'::timestamptz
    GROUP BY
        station, instrument
)
...
SELECT
    ...
    datadays,
    totaldays,
    LEAST(datadays::real / totaldays * 100, 100)::NUMERIC(10,2) as uptime,
    totalimages, 
    totalbytes,
    firstday,
    lastday
